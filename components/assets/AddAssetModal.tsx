...
export default function AddAssetModal({ isOpen, onClose, onSuccess }: AddAssetModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const qrCode = uuidv4()
      const qrUrl = `${window.location.origin}/scan/${qrCode}`
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="flex justify-end space-x-3 pt-4">
                    ...
                    <button
                      type="submit"
                      disabled={isLoading}
                      className="px-4 py-2 text-sm font-medium text-white rounded-md hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed" style={{ backgroundColor: 'var(--accent-fill, #a84605)' }}
                    >
