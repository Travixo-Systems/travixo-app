...
export default function AdminLogoutButton() {
  ...
  async function handleLogout() {
    ...
    // signOut() can fail (network, an already-expired token) and it resolves
    // with an { error } rather than throwing. Navigating anyway used to leave
    // the session cookie in place, and since the proxy bounces a signed-in
    // admin from /login straight back to /admin, reopening the browser logged
    // you in again. Clear this slot's cookie ourselves so the sign-out is
    // real whatever the network did.
    try {
    ...
    // login. A full load also guarantees every Server Component re-renders
    // against the now signed-out session.
