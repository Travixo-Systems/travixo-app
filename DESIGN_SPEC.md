...
| Token | Hex | Usage |
|-------|-----|-------|
| `--page-bg` | `#f6f8fd` | Main content area background |
...
Changes needed:
- Content `<main>` background: `#f6f8fd` (was white)
...
- Same content background `#f6f8fd` applies
...
- 15-30 days: `--text-muted` text on `#f6f8fd` background
...
The page background (#f6f8fd) and card background (#edeff2) should be the default "light" theme. The dark theme (existing) can use the darker palette from the personas doc (#060f10 page, #0a2730 cards) as the "dark" option.
...
For all modals, replace white backgrounds with `--card-bg` (#edeff2) and equipment identity blocks with `--page-bg` (#f6f8fd).
...
- Update the dashboard layout component:
  - Content area background to --page-bg (#f6f8fd)
...
- [ ] Page background is #f6f8fd (not white) on ALL pages
